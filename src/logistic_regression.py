...
def learningCurveLogReg(X, y, X_val, y_val, learning_rate, iterations, reg_param):
    ...
    for subset_size in range(1, m, 10):
        ...
        theta_optimized, _ = trainLogReg(X=X_train, y=y_train, learning_rate=learning_rate, iterations=iterations,
                                         reg_param=reg_param)

        cost_tr, _ = regularizedCostLogReg(X=X_train, theta=theta_optimized, y=y_train, learning_rate=1, reg_param=0)
        error_train.append(np.asscalar(cost_tr))

        cost_te, _ = regularizedCostLogReg(X=X_val, theta=theta_optimized, y=y_val, learning_rate=1, reg_param=0)
