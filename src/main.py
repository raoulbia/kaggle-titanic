...
def model_data(model, dataset, train_file_path, test_file_path, results_file_path,
    ...
    theta_optimized, cost_history = trainLogReg(X=X, y=y, learning_rate=learning_rate, iterations=num_iters,
                                                reg_param=reg_param)
    ...
    if learning_curve == 1:
        ...
        error_train, error_test = learningCurveLogReg(X=X_train, y=y_train, X_val=X_test, y_val=y_test,
